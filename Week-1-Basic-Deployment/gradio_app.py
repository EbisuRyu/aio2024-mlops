...
def plot_bbox(easyocr_results, image):
    boxes = [line[0] for line in easyocr_results]
    texts = [line[1] for line in easyocr_results]
    scores = [line[1] for line in easyocr_results]
...
def extract_ocr(input_image, languages):
    ...
    ocr_result = EASY_OCR.readtext(input_image, slope_ths=0.5, height_ths=1.0, width_ths=1.5)
...
demo = gr.Interface(
    fn=extract_ocr,
    inputs=['image', gr.CheckboxGroup(choices=choices, label='Languages')],
    outputs=['image'] 
)
demo.launch()